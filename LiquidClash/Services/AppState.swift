...
final class AppState {
    ...
    private func applyDefaultProxySelection(persist: Bool) {
        if let node = proxyRegions.flatMap(\.nodes).first {
            selectedNodeId = node.id
            activeNode = node
            proxyService.activeNodeName = node.name
    ...
    func connect() {
        ...
        config.tunEnabled = AppProfile.defaults.bool(forKey: SettingsKey.tunMode)
        config.allowLan = config.tunEnabled && AppProfile.defaults.bool(forKey: SettingsKey.allowLAN)
        ...
        let overlay = ConfigPipeline.OverlayConfig(
            ...
            logLevel: "info",
            allowLan: config.tunEnabled && AppProfile.defaults.bool(forKey: SettingsKey.allowLAN),
        ...
        do {
            ...
            RuntimeCleanup.markCoreStarted(tunEnabled: config.tunEnabled)
        } catch {
            isConnecting = false
    ...
    private func onCoreStarted(api: ClashAPI) {
        ...
        let savedSelection = currentProxySelectionTarget()
        Task {
    ...
    func addSubscription(url: String, name: String, isEnabled: Bool = true) -> String {
        ...
        let sub = SubscriptionInfo(url: url, name: displayName, isEnabled: isEnabled)
    ...
    private func fetchNetworkInfo() async {
        ...
        for _ in 1...8 {
            ...
            // ip-api.com works in rule mode, returns IP + city
        ...
        // ASN type from ipapi.is (direct, not through proxy)
