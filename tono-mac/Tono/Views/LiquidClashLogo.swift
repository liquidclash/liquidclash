...
struct LiquidClashLogo: View {
    ...
    var body: some View {
        ...
        let lightScale: CGFloat = compact ? 1 : (isConnected ? 1.08 : 0.94)
        let lightOpacity: Double = compact ? 1 : (isConnected ? 1 : 0.86)
        ...
        ZStack {
            ...
            Circle()
                ...
                .shadow(
                    color: compact ? .clear : lightColor.opacity(isConnected ? 0.55 : 0.22),
                    radius: compact ? 0 : (isConnected ? 6 : 2)
                )
        }
        .aspectRatio(1, contentMode: .fit)
        .animation(compact ? nil : .easeOut(duration: 0.22), value: isConnected)
    }
}
