...
struct SubscriptionSourcesPanel: View {
    ...
    private func importFromFiles(_ fileURLs: [URL]) {
        isUpdatingSubscription = true
        subscriptionStatus = nil

        Task {
            do {
                guard fileURLs.count <= 20 else {
                    throw SubscriptionError.noNodesFound
                }
                var importedNodes: [ProxyNode] = []
                var importedRules: [RuleItem] = []
                var lastContent = ""
                for fileURL in fileURLs {
                    ...
                    importedNodes.append(contentsOf: nodes)
                    if content.contains("rules:") {
                        importedRules.append(contentsOf: ConfigParser.parseClashYAMLRules(
                            content,
                            source: .subscription
                        ))
                    }
                ...
                let existingNodes = await MainActor.run {
                    appState.proxyRegions
                        .filter { $0.id != "custom" }
                        .flatMap(\.nodes)
                }
                var mergedNodes = existingNodes
                for node in importedNodes {
                    if let index = mergedNodes.firstIndex(where: { $0.name == node.name }) {
                        mergedNodes[index] = node
                    } else {
                        mergedNodes.append(node)
                    }
                }
                _ = try ConfigPipeline.validatedOwnedNodes(mergedNodes)
                let regions = await manager.organizeIntoRegions(mergedNodes)
                ...
                await MainActor.run {
                    ...
                    if !importedRules.isEmpty {
                        ...
                        appState.rules = userRules + importedRules
                    }
                    appState.saveState()
                    ConfigStorage.shared.saveProxyRegions(appState.proxyRegions)
                    // Raw YAML is UI-only in protected mode; retain only the
                    // most recently selected file while credentials live in
                    // the private, sanitized regions store.
                    ConfigStorage.shared.saveRawSubscriptionYAML(lastContent)
                    isUpdatingSubscription = false
                    showTemporaryStatus(String(localized: "✓ Imported \(importedNodes.count) nodes"))
                }
            } catch {
                await MainActor.run {
                    isUpdatingSubscription = false
