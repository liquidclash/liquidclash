...
pub(super) async fn init_dns_config() -> Result<()> {
    ...
    if !dns_path.exists() {
        ...
        help::save_yaml(&dns_path, &default_dns_config, Some("# Tono DNS Config")).await?;
    }
...
pub async fn startup_script() -> Result<()> {
    ...
    // owned runtime may execute at boot.
    logging!(info, Type::Setup, "Tono: startup script execution is disabled");
    Ok(())
}
