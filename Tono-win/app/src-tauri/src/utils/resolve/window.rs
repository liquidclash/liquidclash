...
// 定义默认窗口尺寸常量（与 macOS Tono 一致：920×600 默认，860×540 最小）
const DEFAULT_WIDTH: f64 = 920.0;
const DEFAULT_HEIGHT: f64 = 600.0;

const MINIMAL_WIDTH: f64 = 860.0;
const MINIMAL_HEIGHT: f64 = 540.0;
...
mod tests {
    ...
    fn restored_window_size_rejects_dimensions_below_minimum() {
        assert!(restored_window_size_is_too_small(859, 600));
        assert!(restored_window_size_is_too_small(920, 539));
    }

    #[test]
    fn restored_window_size_accepts_minimum_or_larger_dimensions() {
        assert!(!restored_window_size_is_too_small(860, 540));
        assert!(!restored_window_size_is_too_small(920, 600));
    }
}
