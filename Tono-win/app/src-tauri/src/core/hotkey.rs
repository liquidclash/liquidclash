...
use crate::{config::Config, core::handle, feat};
...
impl Hotkey {
    ...
    fn execute_function(function: HotkeyFunction) {
        match function {
            ...
            // Tono: proxy/mode/profile-affecting shortcuts are disabled
            // (P0-6). Registration is already refused for these; this arm is
            ...
            | HotkeyFunction::ReactivateProfiles => {
                logging!(debug, Type::Hotkey, "Tono: hotkey function is disabled");
            }
            HotkeyFunction::Quit => {
                AsyncHandler::spawn(async move || {
...
impl Hotkey {
    ...
    pub async fn register(&self, hotkey: &str, func: &str) -> Result<()> {
        ...
        // Tono: user-configured hotkeys are limited to the dashboard toggle
        // (P0-6); every other function is refused at registration.
        if !matches!(function, HotkeyFunction::OpenOrCloseDashboard) {
            logging!(info, Type::Hotkey, "Tono: hotkey function {func} is disabled");
            return Ok(());
        }
