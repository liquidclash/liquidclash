...
/// elevation path can otherwise hang forever — an unanswered UAC prompt, a
...
impl<E: RunStateEnv> RunStateStore<E> {
    ...
    /// A store with a shorter privileged-operation cap, for timeout tests.
    #[cfg(test)]
    ...
    ///
    /// The wait is hard-bounded by [`PRIVILEGED_ACTION_TIMEOUT`], but the elevated process cannot
    /// be safely cancelled. If the deadline expires (or this future is cancelled), its outcome
...
impl<E: RunStateEnv> Drop for OperationGuard<'_, E> {
    fn drop(&mut self) {
        if self.store.privileged_outcome_uncertain.load(Ordering::Acquire) {
...
mod tests {
    ...
    /// An environment whose privileged operation never returns — the
    /// real-machine hang (unanswered UAC / wedged installer child).
    #[derive(Debug)]
    struct HungEnv;

    impl RunStateEnv for HungEnv {
        async fn probe_service_version(&self) -> Result<ServiceVersionReply> {
            bail!("no service configured")
        }
        ...
        async fn run_privileged(&self, _action: PendingAction) -> Result<()> {
            std::future::pending().await
        }
    }

    #[tokio::test(start_paused = true)]
    async fn privileged_operation_timeout_quarantines_the_slot_until_restart() {
        let store = RunStateStore::new(HungEnv).with_privileged_timeout(Duration::from_millis(50));
        {
            let _guard = store.begin_operation().expect("slot is free before the hang");
            let error = store.perform(PendingAction::Install).await.unwrap_err();
            let message = format!("{error:#}");
            assert!(message.contains("timed out"), "{message}");
            assert!(message.contains("tono-service-install.exe"), "{message}");
        }
        assert!(
            store.operation_in_flight(),
            "an orphaned privileged helper must keep the operation slot quarantined"
        );
        let error = store
            .begin_operation()
