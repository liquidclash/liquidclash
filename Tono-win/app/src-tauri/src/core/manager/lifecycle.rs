...
impl CoreManager {
    ...
    pub(super) async fn replace_service_core_with_config(&self, config_file: &Path) -> Result<()> {
        run_service_config_replacement_transition(
            ...
            proxy_control::clear,
            || self.stop_core_unprepared_inner(false),
    ...
    pub(crate) async fn controlled_stop_core_inner(&self) -> Result<()> {
        self.controlled_stop_core_with_kill_switch_release(true).await
    }

    async fn controlled_stop_core_with_kill_switch_release(&self, release_kill_switch: bool) -> Result<()> {
        ...
        run_controlled_stop_transition(
            ...
            proxy_control::stop_guard,
            proxy_control::clear,
            || self.stop_core_unprepared_inner(release_kill_switch),
        )
        .await
    }
    ...
    pub async fn restart_core(&self) -> Result<()> {
        ...
        run_core_replacement_transition(
            || self.controlled_stop_core_with_kill_switch_release(false),
            || async {
                self.start_core_inner().await?;
                if matches!(*self.get_running_mode(), RunningMode::NotRunning) {
