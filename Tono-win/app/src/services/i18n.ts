  ...
  try {
    const entries = await Promise.all(
      sections.map(async (section) => {
        // Sections not translated into this language fall back per section to
        // English, then to the global fallback language, instead of dragging
        // the whole language down with one missing file.
        const loader =
          localeLoaders[language]?.[section] ??
          localeLoaders.en?.[section] ??
