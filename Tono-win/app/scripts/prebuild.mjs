...
async function resolveServiceBundle() {
  if (platform === 'win32') {
    ...
    const missing = tonoBinaries.filter((file) => !fs.existsSync(file))
    if (missing.length > 0) {
      throw new Error(
        `Missing Tono Windows Service binaries: ${missing.join(', ')}. Build the sibling service project for ${SIDECAR_HOST} and copy its three release binaries into src-tauri/resources before packaging.`,
      )
    }

    // In the combined Tono-win checkout, refuse to silently package checked-in helpers older
    // than their source. This is intentionally a build error: otherwise an app-only rebuild can
    // claim to contain a Service fix while shipping the previous privileged binaries.
    const serviceSourceRoot = path.resolve(cwd, '..', 'service')
    if (fs.existsSync(path.join(serviceSourceRoot, 'Cargo.toml'))) {
      const sourceFiles = [
        path.join(serviceSourceRoot, 'Cargo.toml'),
        path.join(serviceSourceRoot, 'Cargo.lock'),
        ...glob.sync(path.join(serviceSourceRoot, 'src', '**', '*'), {
          nodir: true,
        }),
      ].filter((file) => fs.existsSync(file))
      const newestSource = Math.max(
        ...sourceFiles.map((file) => fs.statSync(file).mtimeMs),
      )
      const stale = tonoBinaries.filter(
        (file) => fs.statSync(file).mtimeMs < newestSource,
      )
      if (stale.length > 0) {
        throw new Error(
          `Refusing to package stale Tono Service binaries: ${stale.join(', ')}. Rebuild service for ${SIDECAR_HOST} and refresh src-tauri/resources first.`,
        )
      }
    }

    log_success('verified locally built Tono Windows Service bundle')
    return
  }
