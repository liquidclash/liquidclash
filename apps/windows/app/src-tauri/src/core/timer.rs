...
impl Timer {
    ...
    async fn async_task(uid: &String) {
        // Tono: scheduled subscription updates are disabled — the catalog is
        // server-managed and profiles must never reach the core (P0-4).
        logging!(
            debug,
            Type::Timer,
            "Skipping disabled timer profile update for: {}",
            uid
        );
    }
